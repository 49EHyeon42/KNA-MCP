package kna

import (
	"context"
	"errors"
	"fmt"
	"io"
	"net/http"
	"net/http/httptest"
	"reflect"
	"strings"
	"testing"
	"time"

	"github.com/49EHyeon42/KNA-MCP/internal/plantmstns/application"
)

func TestPlantMstnsList(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		if request.URL.Path != plantMstnsListPath {
			t.Errorf("path = %q, want %q", request.URL.Path, plantMstnsListPath)
		}
		if got := request.Header.Get("Accept"); got != "application/xml" {
			t.Errorf("Accept = %q, want application/xml", got)
		}

		query := request.URL.Query()
		wantQuery := map[string]string{
			"serviceKey":   "test+/=",
			"pageNo":       "2",
			"numOfRows":    "1",
			"reqSearchWrd": "test-search-word",
			"reqMnfctYr":   "2014",
		}
		if len(query) != len(wantQuery) {
			t.Errorf("query key count = %d, want %d", len(query), len(wantQuery))
		}
		for key, want := range wantQuery {
			if got := query.Get(key); got != want {
				t.Errorf("query %s = %q, want %q", key, got, want)
			}
		}

		response.Header().Set("Content-Type", "application/xml")
		_, _ = io.WriteString(response, `<?xml version="1.0" encoding="UTF-8"?>
<response>
  <header><resultCode>00</resultCode><resultMsg>NORMAL SERVICE.</resultMsg></header>
  <body>
    <items><item>
      <distrAraDscrt>distribution area description</distrAraDscrt>
      <minitrTpcdNm>miniature type code name</minitrTpcdNm>
      <plantBrdgFomTpcdNm>plant breeding form type code name</plantBrdgFomTpcdNm>
      <plantGnrlNm>plant general name</plantGnrlNm>
      <plantMinitrAthrNm>plant miniature author name</plantMinitrAthrNm>
      <plantMinitrMnfctMonth>plant miniature manufacture month</plantMinitrMnfctMonth>
      <plantMinitrMnfctYr>plant miniature manufacture year</plantMinitrMnfctYr>
      <plantMinitrPsinsNm>plant miniature possession name</plantMinitrPsinsNm>
      <plantSpecsScnm>plant species scientific name</plantSpecsScnm>
      <rrnssPlantYn>rarity plant yes or no</rrnssPlantYn>
      <spcltPlantYn>specialty plant yes or no</spcltPlantYn>
    </item></items>
    <numOfRows>1</numOfRows><pageNo>2</pageNo><totalCount>7</totalCount>
  </body>
</response>`)
	}))
	defer server.Close()

	client, err := NewClient("test%2B%2F%3D")
	if err != nil {
		t.Fatal(err)
	}
	client.baseURL = server.URL

	got, err := client.PlantMstnsList(context.Background(), application.PlantMstnsListQuery{
		PageNo:       2,
		NumOfRows:    1,
		ReqSearchWrd: "test-search-word",
		ReqMnfctYr:   "2014",
	})
	if err != nil {
		t.Fatal(err)
	}

	want := application.PlantMstnsListResult{
		Items: []application.PlantMstnsListItem{{
			DistrAraDscrt:         "distribution area description",
			MinitrTpcdNm:          "miniature type code name",
			PlantBrdgFomTpcdNm:    "plant breeding form type code name",
			PlantGnrlNm:           "plant general name",
			PlantMinitrAthrNm:     "plant miniature author name",
			PlantMinitrMnfctMonth: "plant miniature manufacture month",
			PlantMinitrMnfctYr:    "plant miniature manufacture year",
			PlantMinitrPsinsNm:    "plant miniature possession name",
			PlantSpecsScnm:        "plant species scientific name",
			RrnssPlantYn:          "rarity plant yes or no",
			SpcltPlantYn:          "specialty plant yes or no",
		}},
		NumOfRows:  1,
		PageNo:     2,
		TotalCount: 7,
	}
	if !reflect.DeepEqual(got, want) {
		t.Errorf("result = %#v, want %#v", got, want)
	}
}

func TestPlantMstnsListReturnsEmptyItems(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		for _, key := range []string{"reqSearchWrd", "reqMnfctYr"} {
			if _, exists := request.URL.Query()[key]; exists {
				t.Errorf("%s query was sent for an empty value", key)
			}
		}
		_, _ = io.WriteString(response, `<response><header><resultCode>00</resultCode></header><body><items/><numOfRows>1</numOfRows><pageNo>1</pageNo><totalCount>0</totalCount></body></response>`)
	}))
	defer server.Close()

	client, err := NewClient("test-key")
	if err != nil {
		t.Fatal(err)
	}
	client.baseURL = server.URL

	result, err := client.PlantMstnsList(context.Background(), application.PlantMstnsListQuery{PageNo: 1, NumOfRows: 1})
	if err != nil {
		t.Fatal(err)
	}
	if len(result.Items) != 0 || result.TotalCount != 0 {
		t.Errorf("result = %#v, want empty result", result)
	}
}

func TestPlantMstnsListReturnsDocumentedAPIErrors(t *testing.T) {
	tests := []struct {
		code    string
		message string
	}{
		{code: "02", message: "DB_ERROR"},
		{code: "03", message: "NODATA_ERROR"},
		{code: "05", message: "SERVICETIME_OUT"},
		{code: "10", message: "INVALID_REQUEST_PARAMETER_ERROR"},
		{code: "11", message: "NO_MANDATORY_REQUEST_PARAMETERS_ERROR"},
		{code: "21", message: "TEMPORARILY_DISABLE_THE_SERVICEKEY_ERROR"},
		{code: "33", message: "UNSIGNED_CALL_ERROR"},
	}

	for _, test := range tests {
		t.Run(test.code, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, _ *http.Request) {
				_, _ = fmt.Fprintf(response, `<response><header><resultCode>%s</resultCode></header></response>`, test.code)
			}))
			defer server.Close()

			client, err := NewClient("test-key")
			if err != nil {
				t.Fatal(err)
			}
			client.baseURL = server.URL

			_, err = client.PlantMstnsList(context.Background(), application.PlantMstnsListQuery{PageNo: 1, NumOfRows: 1})
			var apiError *PlantMstnsListError
			if !errors.As(err, &apiError) {
				t.Fatalf("error = %v, want *PlantMstnsListError", err)
			}
			if apiError.HTTPStatus != http.StatusOK || apiError.Code != test.code || apiError.Message != test.message {
				t.Errorf("error = %#v", apiError)
			}
		})
	}
}

func TestPlantMstnsListReturnsGatewayError(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, _ *http.Request) {
		response.WriteHeader(http.StatusUnauthorized)
		_, _ = io.WriteString(response, `<OpenAPI_ServiceResponse><cmmMsgHeader><errMsg>SERVICE_KEY_IS_NULL</errMsg><returnAuthMsg>서비스 접근거부</returnAuthMsg><returnReasonCode>20</returnReasonCode></cmmMsgHeader></OpenAPI_ServiceResponse>`)
	}))
	defer server.Close()

	client, err := NewClient("test-key")
	if err != nil {
		t.Fatal(err)
	}
	client.baseURL = server.URL

	_, err = client.PlantMstnsList(context.Background(), application.PlantMstnsListQuery{PageNo: 1, NumOfRows: 1})
	var apiError *PlantMstnsListError
	if !errors.As(err, &apiError) {
		t.Fatalf("error = %v, want *PlantMstnsListError", err)
	}
	if apiError.HTTPStatus != http.StatusUnauthorized || apiError.Code != "20" || apiError.Message != "SERVICE_KEY_IS_NULL: 서비스 접근거부" {
		t.Errorf("error = %#v", apiError)
	}
}

func TestPlantMstnsListReturnsResponseErrors(t *testing.T) {
	tests := []struct {
		name       string
		statusCode int
		body       string
		wantError  string
	}{
		{name: "unexpected HTTP status", statusCode: http.StatusBadGateway, body: `<response/>`, wantError: "plantMstnsList: unexpected HTTP status 502 Bad Gateway"},
		{name: "invalid XML", statusCode: http.StatusOK, body: `<response>`, wantError: "plantMstnsList: decode response"},
		{name: "missing result code", statusCode: http.StatusOK, body: `<response/>`, wantError: "plantMstnsList: response missing resultCode"},
	}

	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, _ *http.Request) {
				response.WriteHeader(test.statusCode)
				_, _ = io.WriteString(response, test.body)
			}))
			defer server.Close()

			client, err := NewClient("test-key")
			if err != nil {
				t.Fatal(err)
			}
			client.baseURL = server.URL

			_, err = client.PlantMstnsList(context.Background(), application.PlantMstnsListQuery{PageNo: 1, NumOfRows: 1})
			if err == nil || !strings.Contains(err.Error(), test.wantError) {
				t.Errorf("error = %v, want containing %q", err, test.wantError)
			}
		})
	}
}

func TestPlantMstnsListLive(t *testing.T) {
	serviceKey := requireLiveServiceKey(t)

	client, err := NewClient(serviceKey)
	if err != nil {
		t.Fatal(err)
	}

	for _, test := range []struct {
		name           string
		reqSearchWrd   string
		reqMnfctYr     string
		plantGnrlNm    string
		plantSpecsScnm string
	}{
		{name: "without optional conditions"},
		{name: "exact Korean name", reqSearchWrd: "소나무", plantGnrlNm: "소나무"},
		{name: "partial Korean name", reqSearchWrd: "나무", plantGnrlNm: "섬나무딸기"},
		{name: "uppercase scientific name", reqSearchWrd: "Pinus", plantSpecsScnm: "pinus"},
		{name: "lowercase scientific name", reqSearchWrd: "pinus", plantSpecsScnm: "pinus"},
		{name: "manufacture year only", reqMnfctYr: "2014"},
		{name: "both optional conditions", reqSearchWrd: "나무", reqMnfctYr: "2014"},
	} {
		t.Run(test.name, func(t *testing.T) {
			ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
			defer cancel()

			result, err := client.PlantMstnsList(ctx, application.PlantMstnsListQuery{
				PageNo:       1,
				NumOfRows:    10,
				ReqSearchWrd: test.reqSearchWrd,
				ReqMnfctYr:   test.reqMnfctYr,
			})
			if err != nil {
				t.Fatal(err)
			}
			if len(result.Items) == 0 || result.TotalCount < len(result.Items) {
				t.Fatalf("plantMstnsList result = %#v, want nonempty result", result)
			}
			matched := test.plantGnrlNm == "" && test.plantSpecsScnm == ""
			for _, item := range result.Items {
				if test.reqSearchWrd != "" && !strings.Contains(strings.ToLower(item.PlantGnrlNm), strings.ToLower(test.reqSearchWrd)) && !strings.Contains(strings.ToLower(item.PlantSpecsScnm), strings.ToLower(test.reqSearchWrd)) {
					t.Errorf("item = %#v, want plantGnrlNm or plantSpecsScnm containing %q", item, test.reqSearchWrd)
				}
				if test.plantGnrlNm != "" && item.PlantGnrlNm == test.plantGnrlNm {
					matched = true
				}
				if test.plantSpecsScnm != "" && strings.Contains(strings.ToLower(item.PlantSpecsScnm), test.plantSpecsScnm) {
					matched = true
				}
				if test.reqMnfctYr != "" && item.PlantMinitrMnfctYr != test.reqMnfctYr {
					t.Errorf("plantMinitrMnfctYr = %q, want %q", item.PlantMinitrMnfctYr, test.reqMnfctYr)
				}
			}
			if !matched {
				t.Errorf("plantMstnsList items = %#v, want matching result", result.Items)
			}
		})
	}

	t.Run("changed page", func(t *testing.T) {
		ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
		defer cancel()

		first, err := client.PlantMstnsList(ctx, application.PlantMstnsListQuery{PageNo: 1, NumOfRows: 1})
		if err != nil {
			t.Fatal(err)
		}
		second, err := client.PlantMstnsList(ctx, application.PlantMstnsListQuery{PageNo: 2, NumOfRows: 1})
		if err != nil {
			t.Fatal(err)
		}
		if len(first.Items) != 1 || len(second.Items) != 1 || first.TotalCount != second.TotalCount || first.TotalCount < 2 || reflect.DeepEqual(first.Items[0], second.Items[0]) {
			t.Errorf("first = %#v, second = %#v, want distinct pages", first, second)
		}
	})

	t.Run("without result", func(t *testing.T) {
		ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
		defer cancel()

		result, err := client.PlantMstnsList(ctx, application.PlantMstnsListQuery{PageNo: 1, NumOfRows: 1, ReqSearchWrd: "KNA-MCP-NO-RESULT"})
		if err != nil {
			t.Fatal(err)
		}
		if len(result.Items) != 0 || result.TotalCount != 0 {
			t.Errorf("result = %#v, want empty result", result)
		}
	})

	t.Run("invalid service key", func(t *testing.T) {
		client, err := NewClient("invalid-service-key")
		if err != nil {
			t.Fatal(err)
		}
		ctx, cancel := context.WithTimeout(context.Background(), 60*time.Second)
		defer cancel()

		_, err = client.PlantMstnsList(ctx, application.PlantMstnsListQuery{PageNo: 1, NumOfRows: 1})
		var apiError *PlantMstnsListError
		if !errors.As(err, &apiError) {
			t.Fatalf("error = %v, want *PlantMstnsListError", err)
		}
		if apiError.Code != "30" {
			t.Errorf("error = %#v, want code 30", apiError)
		}
	})
}
