...
type relatedSiteListUseCaseStub struct {
	query  application.RelatedSiteListQuery
	result application.RelatedSiteListResult
	err    error
}

func (s *relatedSiteListUseCaseStub) RelatedSiteList(_ context.Context, query application.RelatedSiteListQuery) (application.RelatedSiteListResult, error) {
	s.query = query
	return s.result, s.err
}

func TestRelatedSiteListTool(t *testing.T) {
	...
	addRelatedSiteListTool(server, useCase)
	...
	checkToolInputSchema(t, ctx, clientSession, "lvbng_service_related_site_list",
		map[string]string{
			"pageNo":    "페이지번호 (1 이상)",
	...
	checkToolDescription(t, ctx, clientSession, "lvbng_service_related_site_list", "산림청 국립수목원 생물관련사이트 목록을 조회합니다.")
	...
	wantQuery := application.RelatedSiteListQuery{PageNo: 2, NumOfRows: 10}
	...
	checkToolOutputSchema(t, ctx, clientSession, "lvbng_service_related_site_list",
		map[string]string{
			"items":      "조회 결과 목록",
