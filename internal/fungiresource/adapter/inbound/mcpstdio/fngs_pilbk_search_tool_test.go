...
type fngsPilbkSearchUseCaseStub struct {
	query  application.FngsPilbkSearchQuery
	result application.FngsPilbkSearchResult
	err    error
}

func (s *fngsPilbkSearchUseCaseStub) FngsPilbkSearch(_ context.Context, query application.FngsPilbkSearchQuery) (application.FngsPilbkSearchResult, error) {
	s.query = query
	return s.result, s.err
}

func TestFngsPilbkSearchTool(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	useCase := &fngsPilbkSearchUseCaseStub{result: application.FngsPilbkSearchResult{
	...
	addFngsPilbkSearchTool(server, useCase)
	...
	checkToolInputSchema(t, ctx, clientSession, "fungi_resource_fngs_pilbk_search",
		map[string]string{
			"pageNo":       "페이지번호 (1 이상)",
			"numOfRows":    "한 페이지 결과 수 (1 이상)",
			"reqSearchWrd": "검색할 버섯의 국명 또는 학명",
		},
		[]string{"pageNo", "numOfRows"},
	)
	checkToolDescription(t, ctx, clientSession, "fungi_resource_fngs_pilbk_search", "산림청 국립수목원 버섯도감 목록을 검색합니다.")
	...
	wantQuery := application.FngsPilbkSearchQuery{
		PageNo:       2,
		NumOfRows:    10,
		ReqSearchWrd: "test-search-word",
	}
	...
	checkToolOutputSchema(t, ctx, clientSession, "fungi_resource_fngs_pilbk_search",
		map[string]string{
			"items":      "조회 결과 목록",
