...
func TestPlantRareList(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		if request.URL.Path != "/1400119/PlantResource/plantRareList" {
			t.Errorf("path = %q, want %q", request.URL.Path, "/1400119/PlantResource/plantRareList")
		}
...
<response>
  ...
  <body>
	...
	got, err := client.PlantRareList(context.Background(), application.PlantRareListQuery{
		PageNo:       2,
		NumOfRows:    1,
		ReqSearchWrd: "test-search-word",
	})
	...
	want := application.PlantRareListResult{
...
func TestPlantRareListReturnsEmptyItems(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
	...
	result, err := client.PlantRareList(context.Background(), application.PlantRareListQuery{PageNo: 1, NumOfRows: 1})
	if err != nil {
		t.Fatal(err)
	}
...
func TestPlantRareListReturnsDocumentedAPIErrors(t *testing.T) {
	tests := []struct {
		code    string
		message string
	}{
	...
	for _, test := range tests {
		t.Run(test.code, func(t *testing.T) {
			...
			_, err = client.PlantRareList(context.Background(), application.PlantRareListQuery{PageNo: 1, NumOfRows: 1})
			var apiError *PlantRareListError
			if !errors.As(err, &apiError) {
				t.Fatalf("error = %v, want *PlantRareListError", err)
			}
...
func TestPlantRareListReturnsGatewayError(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, _ *http.Request) {
...
func TestPlantRareListReturnsResponseErrors(t *testing.T) {
	tests := []struct {
		name       string
		statusCode int
		body       string
		...
		{name: "unexpected HTTP status", statusCode: http.StatusBadGateway, body: `<response/>`, wantError: "plantRareList: unexpected HTTP status 502 Bad Gateway"},
		{name: "invalid XML", statusCode: http.StatusOK, body: `<response>`, wantError: "plantRareList: decode response"},
		{name: "missing result code", statusCode: http.StatusOK, body: `<response/>`, wantError: "plantRareList: response missing resultCode"},
	}
...
func TestPlantRareListLive(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if len(result.Items) == 0 {
				t.Fatal("plantRareList returned no items")
			}
			...
			if !matched {
				t.Errorf("plantRareList items = %#v, want matching result", result.Items)
			}
		})
	}
	...
	t.Run("changed page", func(t *testing.T) {
		...
		second, err := client.PlantRareList(ctx, application.PlantRareListQuery{PageNo: 2, NumOfRows: 1})
		if err != nil {
			t.Fatal(err)
		}
