...
func TestPlantSeedUnitListTool(t *testing.T) {
	...
	checkToolInputSchema(t, ctx, clientSession, "plant_resource_plant_seed_unit_list",
		map[string]string{
			...
			"reqSeedSpecsId": "검색할 종자의 종자종ID (plantSeedSearch 결과의 seedSpecsId)",
		},
	...
	checkToolOutputSchema(t, ctx, clientSession, "plant_resource_plant_seed_unit_list",
		map[string]string{
			"items":      "조회 결과 목록",
			...
			"sdwghWeght":       "천립중무게",
			"seedAdmcn":        "종자기건함수율",
			"seedCllctPlace":   "종자수집장소",
			"seedHoldGrainCnt": "종자보유립수",
			"seedHoldQntt":     "종자보유량",
			"seedNo":           "종자번호",
			"seedSpecsId":      "종자종ID",
			"storeChrcrTpcdNm": "저장특성",
			"vtlfct":           "활력률",
			"vtlfctTestYr":     "활력률테스트년도",
		})
