...
type plantWordListUseCaseStub struct {
	query  application.PlantWordListQuery
	result application.PlantWordListResult
	err    error
}

func (s *plantWordListUseCaseStub) PlantWordList(_ context.Context, query application.PlantWordListQuery) (application.PlantWordListResult, error) {
	s.query = query
	return s.result, s.err
}

func TestPlantWordListTool(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	useCase := &plantWordListUseCaseStub{result: application.PlantWordListResult{
		Items: []application.PlantWordListItem{{
			EnglsWrdNm: "English word name",
			KrnWrdNm:   "Korean word name",
			PrfcnWrdNm: "refined word name",
	...
	AddTools(server, UseCases{PlantWordList: useCase})
	...
	wantQuery := application.PlantWordListQuery{
		PageNo:       2,
		NumOfRows:    10,
		ReqSearchWrd: "test-search-word",
	}
