...
func TestScnmSearch(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		if request.URL.Path != "/1400119/KfniService2/scnmSearch" {
			t.Errorf("path = %q, want %q", request.URL.Path, "/1400119/KfniService2/scnmSearch")
		}
...
<response>
  ...
  <body>
    <items><item>
      ...
      <fngsScnmId>fungi scientific name ID</fngsScnmId>
	...
	want := application.ScnmSearchResult{
		Items: []application.ScnmSearchItem{{
			...
			FngsScnmID:        "fungi scientific name ID",
			GenusKorNm:        " ",
			GenusNm:           "genus name",
...
func TestScnmSearchReturnsObservedDateError(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, _ *http.Request) {
		_, _ = io.WriteString(response, `<response><header><resultCode>99</resultCode><resultMsg>ORA-01843: not a valid month</resultMsg></header></response>`)
	}))
	defer server.Close()
	...
	if apiError.Code != "99" || apiError.Message != "ORA-01843: not a valid month" {
		t.Errorf("error = %#v", apiError)
	}
}
...
func TestScnmSearchLive(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "general name only", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqGnrlNm: "망태"}},
		{name: "uppercase scientific name", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqScnm: "Ileodictyon"}},
		{name: "lowercase scientific name", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqScnm: "ileodictyon"}},
		{name: "both name conditions", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqGnrlNm: "가는꼴망태버섯", ReqScnm: "Ileodictyon"}},
		{name: "dateFrom only", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, DateFrom: "20220101"}},
		{name: "dateTo only", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, DateTo: "20221231"}},
		{name: "both date conditions", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, DateFrom: "20220101", DateTo: "20221231"}},
		{name: "all optional conditions", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqGnrlNm: "가는꼴망태버섯", ReqScnm: "Ileodictyon", DateFrom: "20220101", DateTo: "20221231"}},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, item := range result.Items {
				if test.query.ReqGnrlNm != "" && !strings.Contains(item.FngsGnrlNm, test.query.ReqGnrlNm) {
					t.Errorf("fngsGnrlNm = %q, want containing %q", item.FngsGnrlNm, test.query.ReqGnrlNm)
				}
				if test.query.ReqScnm != "" && !strings.Contains(strings.ToLower(item.FngsScnm), strings.ToLower(test.query.ReqScnm)) {
					t.Errorf("fngsScnm = %q, want containing %q", item.FngsScnm, test.query.ReqScnm)
				}
	...
	t.Run("invalid date", func(t *testing.T) {
		...
		_, err := client.ScnmSearch(ctx, application.ScnmSearchQuery{PageNo: 1, NumOfRows: 1, DateFrom: "20241301"})
		var apiError *ScnmSearchError
...
func checkLastUpdtDtm(t *testing.T, value, dateFrom, dateTo string) {
	...
	if dateTo != "" {
		...
		if got.After(to) {
			t.Errorf("lastUpdtDtm = %q, want on or before %s", value, dateTo)
		}
	}
}
