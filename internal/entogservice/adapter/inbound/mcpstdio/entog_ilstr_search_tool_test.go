...
type entogIlstrSearchUseCaseStub struct {
	query  application.EntogIlstrSearchQuery
	result application.EntogIlstrSearchResult
	err    error
}

func (s *entogIlstrSearchUseCaseStub) EntogIlstrSearch(_ context.Context, query application.EntogIlstrSearchQuery) (application.EntogIlstrSearchResult, error) {
	s.query = query
	return s.result, s.err
}

func TestEntogIlstrSearchTool(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	useCase := &entogIlstrSearchUseCaseStub{result: application.EntogIlstrSearchResult{
		Items: []application.EntogIlstrSearchItem{{
			Btnc: "btnc", CprtCtnt: "cprtCtnt", DetailYn: "detailYn",
			EntogOfnmKrlngNm: "entogOfnmKrlngNm", EntogOfnmScnmID: "entogOfnmScnmId", EntogPilbkNo: "entogPilbkNo",
			FamilyKorNm: "familyKorNm", FamilyNm: "familyNm", GenusKorNm: " ", GenusNm: "genusNm",
			ImgURL: "NONE", OrdKorNm: "ordKorNm", OrdNm: "ordNm",
		}},
		NumOfRows: 10, PageNo: 2, TotalCount: 21,
	}}
	clientTransport, serverTransport := mcp.NewInMemoryTransports()
	server := mcpserver.NewServer()
	addEntogIlstrSearchTool(server, useCase)
	...
	tool := tools.Tools[0]
	if tool.Name != "entog_service_entog_ilstr_search" || tool.Description != "산림청 국립수목원 내구강도감 목록을 검색합니다." {
	...
	wantQuery := application.EntogIlstrSearchQuery{St: "2", Sw: "test-search-word", NumOfRows: 10, PageNo: 2}
	...
	wantItemDescriptions := map[string]string{
		"btnc": "학명", "cprtCtnt": "저작권", "detailYn": "상세정보유무", "entogOfnmKrlngNm": "국명",
		"entogOfnmScnmId": "학명ID", "entogPilbkNo": "도감번호", "familyKorNm": "과국명", "familyNm": "과명",
		"genusKorNm": "속국명", "genusNm": "속명", "imgUrl": "이미지URL", "ordKorNm": "목국명", "ordNm": "목명",
	}
