...
type oldSpcmSearchUseCaseStub struct {
	query  application.OldSpcmSearchQuery
	result application.OldSpcmSearchResult
	err    error
}

func (s *oldSpcmSearchUseCaseStub) OldSpcmSearch(_ context.Context, query application.OldSpcmSearchQuery) (application.OldSpcmSearchResult, error) {
	s.query = query
	return s.result, s.err
}

func TestOldSpcmSearchTool(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	useCase := &oldSpcmSearchUseCaseStub{result: application.OldSpcmSearchResult{
		Items: []application.OldSpcmSearchItem{{
			CprtCtnt: "cprtCtnt", FamlKorNm: "famlKorNm", FamlNm: "famlNm",
			FrstRgstnDtm: "frstRgstnDtm", ImgURL: "imgUrl", LastUpdtDtm: " ",
			PlantGnrlNm: "plantGnrlNm", PlantOldSmplNo: "plantOldSmplNo", PlantSpecsScnm: "plantSpecsScnm",
		}},
		NumOfRows: 10, PageNo: 2, TotalCount: 21,
	}}
	clientTransport, serverTransport := mcp.NewInMemoryTransports()
	server := mcpserver.NewServer()
	addOldSpcmSearchTool(server, useCase)
	...
	tool := tools.Tools[0]
	if tool.Name != "old_plant_service_old_spcm_search" || tool.Description != "산림청 국립수목원 한반도고표본 정보 목록을 조회합니다." {
		t.Errorf("tool = %#v", tool)
	}

	wantInputDescriptions := map[string]string{
		"st":        "검색어 구분 (1: 학명 부분 검색, 2: 학명 일치 검색)",
		"sw":        "검색대상어",
		"dateGbn":   "날짜검색구분 (1: 등록일, 2: 수정일)",
		"dateFrom":  "검색시작일 (dateGbn 입력 시 필수, yyyyMMdd)",
		"dateTo":    "검색종료일 (dateGbn 입력 시 필수, yyyyMMdd)",
	...
	wantQuery := application.OldSpcmSearchQuery{St: "2", Sw: "test-search-word", DateGbn: "1", DateFrom: "20240101", DateTo: "20241231", NumOfRows: 10, PageNo: 2}
	...
	wantItemDescriptions := map[string]string{
		"cprtCtnt": "저작권", "famlKorNm": "과국명", "famlNm": "과명", "frstRgstnDtm": "최초등록일",
		"imgUrl": "이미지URL", "lastUpdtDtm": "최종수정일", "plantGnrlNm": "국명",
		"plantOldSmplNo": "고표본번호", "plantSpecsScnm": "학명",
	}
