...
func TestScnmSearch(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		if request.URL.Path != "/1400119/KiniService/scnmSearch" {
			t.Errorf("path = %q, want %q", request.URL.Path, "/1400119/KiniService/scnmSearch")
		}
...
<response>
  ...
  <body>
    <items><item>
      ...
      <insctScnmId>insect scientific name ID</insctScnmId>
	...
	want := application.ScnmSearchResult{
		Items: []application.ScnmSearchItem{{
			...
			InsctScnmID:       "insect scientific name ID",
...
func TestScnmSearchLive(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "general name only", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqGnrlNm: "호랑나비"}},
		{name: "uppercase scientific name", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqScnm: "Papilio"}},
		{name: "lowercase scientific name", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqScnm: "papilio"}},
		{name: "both name conditions", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqGnrlNm: "호랑나비", ReqScnm: "Papilio"}},
		...
		{name: "all optional conditions", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqGnrlNm: "호랑나비", ReqScnm: "Papilio", DateFrom: "20190101", DateTo: "20201231"}},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, item := range result.Items {
				if test.query.ReqGnrlNm != "" && !strings.Contains(item.InsctGnrlNm, test.query.ReqGnrlNm) {
					t.Errorf("insctGnrlNm = %q, want containing %q", item.InsctGnrlNm, test.query.ReqGnrlNm)
				}
				if test.query.ReqScnm != "" && !strings.Contains(strings.ToLower(item.InsctSpecsScnm), strings.ToLower(test.query.ReqScnm)) {
					t.Errorf("insctSpecsScnm = %q, want containing %q", item.InsctSpecsScnm, test.query.ReqScnm)
				}
