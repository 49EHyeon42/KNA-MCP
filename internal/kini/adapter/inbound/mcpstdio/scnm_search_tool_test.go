...
func TestScnmSearchTool(t *testing.T) {
	...
	inputDescriptions := map[string]string{
		...
		"reqGnrlNm": "검색하려는 곤충국명(곤충명) (부분 문자열 검색)",
		"reqScnm":   "검색하려는 곤충학명 (대소문자를 구분하지 않는 부분 문자열 검색)",
		"dateFrom":  "최종수정일 이후 정보 (yyyyMMdd)",
		"dateTo":    "최종수정일 이전 정보 (yyyyMMdd)",
	}
	checkToolInputSchema(t, ctx, clientSession, "kini_scnm_search", inputDescriptions, []string{"pageNo", "numOfRows"})
	checkToolDescription(t, ctx, clientSession, "kini_scnm_search", "산림청 국립수목원 국가표준곤충목록의 곤충 학명 목록을 조회합니다.")
	...
	checkToolOutputSchema(t, ctx, clientSession, "kini_scnm_search", outputDescriptions, itemDescriptions)
