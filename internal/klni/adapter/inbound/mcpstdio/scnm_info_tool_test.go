...
func TestScnmInfoTool(t *testing.T) {
	...
	checkToolInputSchema(t, ctx, clientSession, "klni_scnm_info", map[string]string{
		"reqLchnScnmId": "지의류 학명ID (scnmSearch 결과의 lchnScnmId)",
	}, []string{"reqLchnScnmId"})
	checkToolDescription(t, ctx, clientSession, "klni_scnm_info", "산림청 국립수목원 국가표준지의류목록의 지의류 학명 상세 정보를 조회합니다.")

	result, err := clientSession.CallTool(ctx, &mcp.CallToolParams{
		Name:      "klni_scnm_info",
		Arguments: map[string]any{"reqLchnScnmId": "test-lichen-scientific-name-id"},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if want := (application.ScnmInfoQuery{ReqLchnScnmID: "test-lichen-scientific-name-id"}); useCase.query != want {
	...
	result, err = clientSession.CallTool(ctx, &mcp.CallToolParams{
		Name:      "klni_scnm_info",
		Arguments: map[string]any{"reqLchnScnmId": "not-found"},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func checkScnmInfoOutputSchema(t *testing.T, ctx context.Context, session *mcp.ClientSession) {
	...
	itemDescriptions := map[string]string{
		...
		"lastUpdtDtm":       "최종수정일",
		"lchnGnrlNm":        "지의류 추천 국명(지의류명)",
		"lchnGnrlNm2":       "지의류 비추천 국명",
		"lchnScnm":          "지의류 학명",
		"lchnScnmId":        "지의류 학명ID",
		"ordscLtrtrNm":      "지의류 학명 기재문",
		"rmrk":              "비고",
	}

	for _, tool := range result.Tools {
		if tool.Name != "klni_scnm_info" {
			continue
		}
		schema, ok := tool.OutputSchema.(map[string]any)
		if !ok {
			t.Fatalf("tool klni_scnm_info output schema = %#v", tool.OutputSchema)
		}
		properties, ok := schema["properties"].(map[string]any)
		if !ok {
			t.Fatalf("tool klni_scnm_info output schema properties = %#v", schema["properties"])
		}
		checkKeys(t, properties, "item")
		checkPropertyDescriptions(t, "klni_scnm_info", properties, map[string]string{"item": "상세 조회 결과"})

		itemProperty, ok := properties["item"].(map[string]any)
		if !ok {
			t.Fatalf("tool klni_scnm_info item property = %#v", properties["item"])
		}
		itemProperties, nullable := nullableObjectProperties(itemProperty)
		if !nullable {
			t.Errorf("tool klni_scnm_info item schema is not nullable: %#v", itemProperty)
		}
		if itemProperties == nil {
			t.Fatalf("tool klni_scnm_info item schema has no object properties: %#v", itemProperty)
		}
		checkKeys(t, itemProperties, mapKeys(itemDescriptions)...)
		checkPropertyDescriptions(t, "klni_scnm_info item", itemProperties, itemDescriptions)
		return
	}
	t.Fatal("tool klni_scnm_info not found")
}
