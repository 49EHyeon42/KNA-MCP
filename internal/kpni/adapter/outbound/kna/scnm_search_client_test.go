...
func TestScnmSearch(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, request *http.Request) {
		if request.URL.Path != "/1400119/KpniService/scnmSearch" {
			t.Errorf("path = %q, want %q", request.URL.Path, "/1400119/KpniService/scnmSearch")
		}
...
<response>
  ...
  <body>
    <items><item>
      ...
      <plantScnmId>plant scientific name ID</plantScnmId>
      <plantSpecsClsscCdNm>plant species classification code name</plantSpecsClsscCdNm>
      <plantSpecsScnm>plant species scientific name</plantSpecsScnm>
      <stpltScnmRltnCdNm>standard plant scientific name relation code name</stpltScnmRltnCdNm>
      <subClassKorNm>subclass Korean name</subClassKorNm>
      <subClassNm>subclass name</subClassNm>
    </item></items>
...
func TestScnmSearchReturnsObservedDateError(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(response http.ResponseWriter, _ *http.Request) {
		_, _ = io.WriteString(response, `<response><header><resultCode>99</resultCode><resultMsg>ORA-01841: invalid year</resultMsg></header></response>`)
	}))
	defer server.Close()
	...
	if apiError.Code != "99" || apiError.Message != "ORA-01841: invalid year" {
		t.Errorf("error = %#v", apiError)
	}
}
...
func TestScnmSearchLive(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "general name only", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqGnrlNm: "소나무"}},
		{name: "uppercase scientific name", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqScnm: "Pinus"}},
		{name: "lowercase scientific name", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqScnm: "pinus"}},
		{name: "both name conditions", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, ReqGnrlNm: "소나무", ReqScnm: "Pinus"}},
		{name: "dateFrom only", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, DateFrom: "20240101"}},
		{name: "dateTo only", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, DateTo: "20241231"}},
		{name: "both date conditions", query: application.ScnmSearchQuery{PageNo: 1, NumOfRows: 10, DateFrom: "20240101", DateTo: "20241231"}},
	} {
		t.Run(test.name, func(t *testing.T) {
			...
			for _, item := range result.Items {
				if test.query.ReqGnrlNm != "" && !strings.Contains(item.PlantGnrlNm, test.query.ReqGnrlNm) {
					t.Errorf("plantGnrlNm = %q, want containing %q", item.PlantGnrlNm, test.query.ReqGnrlNm)
				}
				if test.query.ReqScnm != "" && !strings.Contains(strings.ToLower(item.PlantSpecsScnm), strings.ToLower(test.query.ReqScnm)) {
					t.Errorf("plantSpecsScnm = %q, want containing %q", item.PlantSpecsScnm, test.query.ReqScnm)
				}
